...
def sup_cluster(df, form, stopwords_list):
    ...
    for k in k_list:
        ...
        # 'Supervised Clustering': cal k-mean cluster
        # model = KMeans(n_clusters=k, init='k-means++', n_init=1)
        model = KMeans(n_clusters=k, init='k-means++', n_init=10, max_iter=300, tol=0.0001, verbose=0, random_state=42,
        ...
        df_kmean = df.loc[df['Seed'] == '1', ['Kmeans_' + str(k)]]
        ...
        for i in range(k):
            ...
            sum_cluster = df_kmean[df_kmean['Kmeans_' + str(k)] == i].shape[0]
            data = {f'Cluster Number(k-{str(k)})': i,
                    f'# of Seeds(k-{str(k)})': sum_cluster}
...
# for NMF Algo test
"""
def run_NMF(df, stopwords):

    # nltk.download('punkt')
    filename = 'consumer_compliants.csv'
    ...
    def tokenize(text):
        tokens = [word for word in nltk.word_tokenize(text) if
                  (len(word) > 3 and len(word.strip('Xx/')) > 2 and len(re.sub('\d+', '', word.strip('Xx/'))) > 3)]
        tokens = map(str.lower, tokens)
        stems = [stemmer.stem(item) for item in tokens if (item not in stop_words)]
        return stems

    vectorizer = TfidfVectorizer(tokenizer=tokenize, stop_words=None, max_df=0.75, max_features=1000, lowercase=False,
                                 ngram_range=(1, 2))
    tfidf_vectors = vectorizer.fit_transform(X_train.complaints)
    print(tfidf_vectors.A)
